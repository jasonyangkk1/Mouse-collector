...
export default function App() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const prompt = `你是一個專業的股市分析助理。請分析上傳的截圖，並嚴格遵循以下優先級邏輯：

1. **對應關係**：
   - 員工 A：台股分析儀 (Taiwan Stock Analyzer)
   - 員工 B：Momentum Core
   - 員工 C：Zenith Intelligence

2. **評級轉換規則 (核心優先級：顏色 > 文字)**：
   **【嚴格遵守：顏色優先原則】**
   - **'WAIT' (輸出為 WAIT)**：
     * 如果截圖區域或右側數字顯示為「黃色」或「橙色」，**無論文字寫什麼**（例如：即使寫著「可繼續持有」、「逢低布局」等），一律判定為 'WAIT' (X)。
     * 關鍵詞：觀望、待站穩、持股續抱但區間震盪、暫時觀望。
   - **'ALERT' (輸出為 ALERT)**：
     * 如果截圖區域顯示為「紅色」，**一律判定為 'ALERT' (X)**。
     * 關鍵詞：警告、避開、減碼、空頭、不建議介入。
   - **'BUY' (輸出為 BUY)**：
     * 只有在截圖區域顯示為「純正綠色」且明確寫著「買進」、「可以買進」或「強勢多頭」時，才判定為 'BUY' (O)。

3. **特別注意 (員工 A - 台股分析儀)**：
   - 該工具常出現「可繼續持有」但背景/數字是黃色的情況，請務必將其歸類為 'WAIT' (X)。
   - 請檢查每行右側顯示的「建議買價」或「價格區間」的顏色，那通常是正確評級的顏色指標。

請提取每檔股票的代碼 (ID)、名稱 (Name) 以及這三位員工的評級 (Rating)。
輸出格式必須是 JSON 陣列，每個物件 represent a single stock.`;
